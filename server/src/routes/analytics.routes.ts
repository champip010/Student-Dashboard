...
router.get('/class/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const assignmentCompletionRate =
      classData.assignments.length > 0
        ? (classData.assignments.reduce(
            (sum, a) =>
              sum +
              a.submissions.filter((s) => s.status === 'SUBMITTED' || s.status === 'LATE').length,
            0
          ) /
